...
void rtg_main(void) {
	...
	while(1) {
		...
		if (do_send) {
			HAL_StatusTypeDef status = HAL_SPI_Receive_DMA(SPI_S, (uint8_t*) buffer1, len_msg);
			if (status != HAL_OK) {
				printf("Failed to receiveIT on SPI_S: %d, error 0x%"PRIX32"\r\n", status, HAL_SPI_GetError(SPI_S));
			}

			printf("SPI_M sends message: %s\r\n", message);
			status = HAL_SPI_Transmit_DMA(SPI_M, (uint8_t*) message, len_msg);
			if (status != HAL_OK) {
		...
		if (spiS_received) {
			...
			HAL_StatusTypeDef status = HAL_SPI_TransmitReceive_DMA(SPI_S, (uint8_t*) buffer1, (uint8_t*) buffer1, len_msg);
			if (status != HAL_OK) {
				printf("Failed to tr-recIT on SPI_S: %d, error 0x%"PRIX32"\r\n", status, HAL_SPI_GetError(SPI_S));
			}

			printf("SPI_S sends message: %s\r\n", buffer1);
			status = HAL_SPI_Receive_DMA(SPI_M, (uint8_t*) buffer2, len_msg);
			if (status != HAL_OK) {
